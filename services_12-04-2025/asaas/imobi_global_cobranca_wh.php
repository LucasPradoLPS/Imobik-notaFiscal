...
// Webhook v.2.1 15-04-2024
// Webhook v.2.1 18-09-2024
//  
...
try
{
	...
	$stmt->execute([':service' => 'imobi_global_cobranca_wh.php',
	...
	{	
		...
		// Repasse de aluguel
		$select  = $pdo->query( "SELECT SUM(repassevalor) AS repassevalor FROM financeiro.faturadetalhe WHERE idfatura = {$idfatura}" );
		$total   = $select->fetch(PDO::FETCH_OBJ);
		$repassevalor = $total->repassevalor;
		
		if($contrato->aluguelgarantido == 'N' AND $repassevalor > 0 )
		{
			// Recupera locadores
			...
			foreach($locadores AS $locador) // percorre locadores
			{
				if($locador->cota > 0) // Só preenche quando locador tem cota
				{
					...
					foreach( $itens AS $item)
					{
						...
						$repasse = (($item->valor * $item->qtde) - ($item->comissaovalor * $item->qtde) - $item->desconto ) * $locador->cota / 100;
						$fatura_valor_total += $repasse;
