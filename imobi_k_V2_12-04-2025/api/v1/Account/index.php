...
function save_data_a($f, $arr){ file_put_contents($f, json_encode($arr, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)); }

$data = load_data_a($storage);
...
function sanitize_input_a(array $input): array {
    $blacklist = ['class','method','entity','controller','token_mobile','token','_'];
    foreach ($blacklist as $k) { if (array_key_exists($k, $input)) unset($input[$k]); }
    foreach ($input as $k => $v) { if (is_string($k) && trim($k) === '') unset($input[$k]); }
    return $input;
}
...
if ($method === 'POST') {
    ...
    if (is_array($input)) { $input = sanitize_input_a($input); }
    @file_put_contents($dbg, date('c') . " | post-input=" . json_encode($input, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) . "\n", FILE_APPEND);
    // generate id
    $next = 1; foreach ($data as $it) { if (preg_match('/a(\d+)/',$it['id'],$m)) $next = max($next, intval($m[1])+1); }
    $new = array_merge(['id'=>'a'.$next], $input);
    $data[] = $new; save_data_a($storage,$data);
...
if ($method === 'PUT') {
    ...
    foreach ($data as &$item) if ($item['id'] == $id) { $item = array_merge($item, $input); save_data_a($storage,$data); echo json_encode(['status'=>200,'success'=>true,'data'=>$item]); exit; }
    http_response_code(404); echo json_encode(['status'=>404,'success'=>false,'error'=>'not found']); exit;
}
if ($method === 'DELETE') {
    if (!$id) { http_response_code(400); echo json_encode(['status'=>400,'success'=>false,'error'=>'missing id']); exit; }
    $new = array_filter($data, function($it) use($id){ return $it['id'] !== $id; });
    save_data_a($storage, array_values($new));
