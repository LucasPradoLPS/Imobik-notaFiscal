...
function save_data_l($f, $arr){ file_put_contents($f, json_encode($arr, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)); }

$data = load_data_l($storage);
...
if ($method === 'POST') {
    ...
    $next = 1; foreach ($data as $it) { if (preg_match('/l(\d+)/',$it['id'],$m)) $next = max($next, intval($m[1])+1); }
    $new = array_merge(['id'=>'l'.$next], $input);
    $data[] = $new; save_data_l($storage,$data);
...
if ($method === 'PUT') {
    ...
    foreach ($data as &$item) if ($item['id'] == $id) { $item = array_merge($item, $input); save_data_l($storage,$data); echo json_encode(['status'=>200,'success'=>true,'data'=>$item]); exit; }
    http_response_code(404); echo json_encode(['status'=>404,'success'=>false,'error'=>'not found']); exit;
}
...
if ($method === 'DELETE') {
    ...
    save_data_l($storage, array_values($new));
