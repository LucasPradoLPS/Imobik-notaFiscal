...
class ContratoAtualizacaoForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    private static $formName = 'form_ContratoAtualizacaoForm';
    ...
    public function __construct( $param )
    {
        ...
        $this->form->setFormTitle("Atualização Monetária de Contrato");
        ...
        $ato = new TEntry('ato');
        $bhelper_63fa1edf3d7de = new BHelper();
        ...
        $bhelper_655d0016dc5f5 = new BHelper();
        $termos = new THtmlEditor('termos');

        $idcontrato->addValidation("Idcontrato", new TRequiredValidator()); 
        $idcontratoalteracaotipo->addValidation("Tipo de Atualização", new TRequiredValidator()); 
        $aluguel->addValidation("Valor do Aluguel", new TRequiredValidator()); 
        $termos->addValidation("Termos", new TRequiredValidator()); 

        $ato->setValue('Atualização Monetária');
        $idtemplate->setTip("<strong>Modelo</strong><br />Tipo: <i>Contrato</i> <br />Tabela: <i>Contrato</i>");
        $bhelper_655d0016dc5f5->enableHover();
        $ato->setEditable(false);
        $idcontrato->setEditable(false);

        $bhelper_63fa1edf3d7de->setSide("left");
        $bhelper_655d0016dc5f5->setSide("left");

        $bhelper_63fa1edf3d7de->setIcon(new TImage("fas:question #FD9308"));
        $bhelper_655d0016dc5f5->setIcon(new TImage("fas:exclamation-circle #fa931f"));

        $bhelper_655d0016dc5f5->setTitle("Assinatura Eletrônica");
        $bhelper_63fa1edf3d7de->setTitle("Alteração de valores:");

        $bhelper_655d0016dc5f5->setContent("Para possibilitar a assinatura eletrônica, os termos devem conter pelo menos um parágrafo, com aproximadamente 100 toques.");
        $bhelper_63fa1edf3d7de->setContent("<ul><li><strong>Reajuste</strong>: Baseado em índice e prazo acordado no contrato.</li><li><strong>Revisão</strong>: Baseado em valor do mercado;</li></ul>");
        ...
        $jurosmora->setAllowNegative(false);
        ...
        $bhelper_63fa1edf3d7de->setSize('14');
        $bhelper_655d0016dc5f5->setSize('18');
        $idcontratoalteracaotipo->setSize('100%');


        $row1 = $this->form->addFields([new TLabel("Contrato Nº:", '#ff0000', '14px', null, '100%'),$idcontrato,$idcontratoaleracao],[new TLabel("Ato:", null, '14px', null),$ato],[$bhelper_63fa1edf3d7de,new TLabel("Tipo de Atualização:", '#FF0000', '14px', null),$idcontratoalteracaotipo]);
        $row1->layout = [' col-sm-2',' col-sm-4','col-sm-6'];

        $row2 = $this->form->addFields([new TLabel("Valor do Aluguel:", '#FF0000', '14px', null, '100%'),$aluguel],[new TLabel("Taxa Administração Imobiliária:", null, '14px', null, '100%'),$comissao],[new TLabel("Juros (% am)", null, '14px', null, '100%'),$jurosmora],[new TLabel("Multa (%)", null, '14px', null, '100%'),$multamora]);
        ...
        $row4 = $this->form->addFields([$bhelper_655d0016dc5f5,new TLabel("Termos:", '#ff0000', '14px', null),$termos]);
        $row4->layout = [' col-sm-12'];

        // create the form actions
        $btn_onsave = $this->form->addAction("Atualizar", new TAction([$this, 'onSave']), 'fas:comments-dollar #ffffff');
        ...
        $style = new TStyle('right-panel > .container-part[page-name=ContratoAtualizacaoForm]');
    ...
    public static function onToSign($param = null) 
    {
        try 
        {
            ...
            //   throw new Exception('Clique em [Atualizar] antes de enviar para a assinatura');
            ...
            TApplication::loadPage('DocumentoFormToSign','onEnter',['key'=> $param['idcontrato'], 'pdf' => $pdf, 'data' =>'contrato', 'title' =>"Atualização Moetária do Contrato Nº {$param['idcontrato']}" ]);
            TForm::sendData(self::$formName, $param);
            TTransaction::close();
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            $object->aluguelant     = $contrato->aluguel;
            $object->comissaoant    = $contrato->comissao;
            $object->jurosmoraant   = $contrato->jurosmora;
            $object->multamoraant   = $contrato->multamora;
            $object->aluguelatual   = str_replace(['.', ','], ['', '.'], $param['aluguel']) ;
            $object->comissaoatual  = str_replace(['.', ','], ['', '.'], $param['comissao']) ;
            $object->jurosmoraatual = str_replace(['.', ','], ['', '.'], $param['jurosmora']) ;
            $object->multamoraatual = str_replace(['.', ','], ['', '.'], $param['multamora']) ;
            ...
            // Atualiza o contrato
            $aluguelold   = number_format($contrato->aluguel, 2, ',', '.');
            $comissaoold  = number_format($contrato->comissao, 2, ',', '.');
            $jurosmoraold = number_format($contrato->jurosmora, 2, ',', '.');
            $multamoraold = number_format($contrato->multamora, 2, ',', '.');

            $aluguelnew   = $param['aluguel'];
            $comissaonew  = $param['comissao'];
            $jurosmoranew = $param['jurosmora'];
            $multamoranew = $param['multamora'];

            // Atualiza Contrato
            $contrato->aluguel   = str_replace(['.', ','], ['', '.'], $param['aluguel']) ;
            $contrato->comissao  = str_replace(['.', ','], ['', '.'], $param['comissao']) ;
            $contrato->jurosmora = str_replace(['.', ','], ['', '.'], $param['jurosmora']) ;
            $contrato->multamora = str_replace(['.', ','], ['', '.'], $param['multamora']) ;
            $contrato->dtproxreajuste   = Uteis::nextMonths(date("Y-m-d"), 12);
            $contrato->store();
...
*/
                ...
                $object->aluguel = $contrato->aluguel;
                $object->comissao = $contrato->comissao;
                $object->jurosmora = $contrato->jurosmora;
                $object->multamora = $contrato->multamora;
