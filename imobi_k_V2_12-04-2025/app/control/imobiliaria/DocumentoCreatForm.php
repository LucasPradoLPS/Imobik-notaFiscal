...
class DocumentoCreatForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    public function __construct( $param )
    {
        ...
        $this->form->setFormTitle("Redigir Documento");
        ...
        $html->addValidation("Texto", new TRequiredValidator()); 
        ...
        $idtemplate->setTip("<strong>Modelo</strong><br />Tipo: <i>Molde</i> <br />Tabela: <i>Indistinto</i>");
        ...
        $addTitulo->setImage('fas:plus-circle #2ECC71');
        $button_preencher->setImage('fas:file-import #9400D3');
        $button__signatario_fk_iddocumento->setImage('fas:user-plus #2ECC71');
        ...
        $bhelper_63d9710a6794c->setIcon(new TImage("fas:exclamation-triangle #EF4E50"));
        $bhelper_63d96a8fb859b->setIcon(new TImage("fas:exclamation-triangle #EF4E50"));
        ...
        $idtemplate->setSize('calc(100% - 140px)');
        ...
        $row3 = $this->form->addFields([new TLabel("Texto:", '#FF0000', '14px', null, '100%'),$html]);
        $row3->layout = [' col-sm-12'];

        $row4 = $this->form->addContent([new TFormSeparator("<br />Opções do Documento", '#333', '18', '#eee')]);
        $row5 = $this->form->addFields([new TLabel("Data Limite para Assinatura:", null, '14px', null, '100%'),$datelimittosign],[$bhelper_63d932adc08a3,new TLabel("Relembrar (dias):", null, '14px', null),$zsremindereveryndays],[new TLabel("Linguagem:", null, '14px', null),$zslang]);
        $row5->layout = ['col-sm-3','col-sm-3','col-sm-3'];

        $row6 = $this->form->addFields([$bhelper_63d98eb76b10a,new TLabel("Desabilitar todos os E-mails :", null, '14px', null),$disablesigneremails],[$bhelper_63d929fe8378c,new TLabel("Desabilitar os Botões:", null, '14px', null),$signedfileonlyfinished],[$bhelper_63d92d23319f8,new TLabel("Assinar na Ordem de Criação:", null, '14px', null),$zssignatureorderactive]);
        ...
        $row7 = $this->detailFormSignatarioFkIddocumento->addFields([new TFormSeparator("Pessoa(s)", '#333', '18', '#eee')]);
        $row7->layout = [' col-sm-12'];

        $row8 = $this->detailFormSignatarioFkIddocumento->addFields([new TLabel("Pessoa", '#FF0000', '14px', null, '100%'),$idpessoa,$button__signatario_fk_iddocumento,$signatario_fk_iddocumento_idsignatario],[new TLabel("Qualificação", null, '14px', null, '100%'),$signatario_fk_iddocumento_qualification],[new TLabel("Ação:", null, '14px', null, '100%'),$button_adicionar_para_assinar_signatario_fk_iddocumento]);
        $row8->layout = [' col-sm-6',' col-sm-3',' col-sm-3'];

        $row9 = $this->detailFormSignatarioFkIddocumento->addFields([new TFormSeparator("Opções do Signatário", '#333', '18', '#eee')]);
        $row9->layout = [' col-sm-12'];

        $row10 = $this->detailFormSignatarioFkIddocumento->addFields([new TLabel("Modo de Autenticação:", null, '14px', null, '100%'),$signatario_fk_iddocumento_authmode],[$bhelper_63d9710a6794c,new TLabel("Reconhecimento Facial:", null, '14px', null),$signatario_fk_iddocumento_selfievalidationtype],[$bhelper_63d96a8fb859b,new TLabel("WhatsApp Automático:", null, '14px', null),$signatario_fk_iddocumento_sendautomaticwhatsapp],[new TLabel("Bloquear o Nome:", null, '14px', null, '100%'),$signatario_fk_iddocumento_lockname]);
        $row10->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row11 = $this->detailFormSignatarioFkIddocumento->addFields([new TLabel("E-mail Automático:", null, '14px', null, '100%'),$signatario_fk_iddocumento_sendautomaticemail],[new TLabel("Ocultar o E-mail:", null, '14px', null, '100%'),$signatario_fk_iddocumento_hideemail],[new TLabel("Bloquear o E-mail:", null, '14px', null, '100%'),$signatario_fk_iddocumento_lockemail],[new TLabel("Não Solicitar o E-mail:", null, '14px', null, '100%'),$signatario_fk_iddocumento_blankemail]);
        $row11->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row12 = $this->detailFormSignatarioFkIddocumento->addFields([new TLabel("Bloquear Telefone:", null, '14px', null, '100%'),$signatario_fk_iddocumento_lockphone],[new TLabel("Não Solicitar Telefone:", null, '14px', null, '100%'),$signatario_fk_iddocumento_blankphone],[new TLabel("Ocultar Telefone", null, '14px', null, '100%'),$signatario_fk_iddocumento_hidephone],[new TLabel("Solicitar Foto de Doc.:", null, '14px', null, '100%'),$signatario_fk_iddocumento_requiredocumentphoto]);
        $row12->layout = [' col-sm-3',' col-sm-3',' col-sm-3',' col-sm-3'];

        $row13 = $this->detailFormSignatarioFkIddocumento->addFields([new TLabel("Solicitar Selfie:", null, '14px', null, '100%'),$signatario_fk_iddocumento_requireselfiephoto],[new TLabel("Mensagem Customizada:", null, '14px', null, '100%'),$signatario_fk_iddocumento_custommessage]);
        $row13->layout = [' col-sm-3',' col-sm-9'];

        $row14 = $this->detailFormSignatarioFkIddocumento->addFields([new TFormSeparator("Signatários - Listagem", '#333', '18', '#eee')]);
        $row14->layout = [' col-sm-12'];

        $row15 = $this->detailFormSignatarioFkIddocumento->addFields([new THidden('signatario_fk_iddocumento__row__id')]);
        ...
        $action_onEditDetailSignatario = new TDataGridAction(array('DocumentoCreatForm', 'onEditDetailSignatario'));
        ...
        $action_onDeleteDetailSignatario = new TDataGridAction(array('DocumentoCreatForm', 'onDeleteDetailSignatario'));
        ...
        $action_onDeleteDetailSignatario->setImage('far:trash-alt #EF4648');
        ...
        $row16 = $this->form->addFields([$this->detailFormSignatarioFkIddocumento]);
        ...
        $btn_onclear = $this->form->addAction("Limpar Formulário", new TAction([$this, 'onClear']), 'fas:eraser #607D8B');
        ...
        $style = new TStyle('right-panel > .container-part[page-name=DocumentoCreatForm]');
    ...
    public function onToSend($param = null) 
    {
        try 
        {
            ...
            // Enviar documento
            $assinatura = Documento::setDocumentToSing( $object->iddocumento, $file );
