...
class WizardReembolsoList extends TPage
{
    ...
    public function __construct($param = null)
    {
        ...
        $column_faturadetalheitem = new TDataGridColumn('faturadetalheitem', "Item/Descrição", 'left');
        ...
        $column_calculated_6 = new TDataGridColumn('=( ({qtde} * {valor} ) -{desconto}  )', "Total", 'right');

        $column_calculated_6->setTotalFunction( function($values) { 
        ...
        $column_calculated_6->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            if(!$value)
            {
                $value = 0;
        ...
        $this->datagrid->addColumn($column_calculated_6);
        ...
        $panel = new TPanelGroup("{$param['title']}");
...
// Olá mundo
   ...
   /**
     * Load the datagrid with data
     */
    public function onReload1($param = NULL)
    {
        try
        {
            // open a transaction with database 'imobi_producao'
            ...
            {
                $object = unserialize(base64_decode($linha));
                // echo '<pre>' ; print_r($object);echo '</pre>';

                if($param['key'] == $object->repasselocador)
                {
                    $faturadetalheitem = new Faturadetalheitem($object->idfaturadetalheitem, FALSE);
                    // echo '<pre>' ; print_r($object);echo '</pre>';
                    $object->faturadetalheitem = $faturadetalheitem->faturadetalheitem;
