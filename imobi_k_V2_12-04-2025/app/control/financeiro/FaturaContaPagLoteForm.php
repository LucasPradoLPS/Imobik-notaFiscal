...
class FaturaContaPagLoteForm extends TPage
{
    protected $form;
    private $formFields = [];
    ...
    public function __construct( $param )
    {
        ...
        $this->form->setFormTitle("Conta a Pagar - Lançamento");
...
$cotas->enableStepper();
        ...
        $row1 = $bcontainer_643b11be2b6b2->addFields([new TLabel("Pessoa (Sacado):", '#ff0000', '14px', null, '100%'),$idpessoa,$button_],[new TLabel("Cotas:", null, '14px', null, '100%'),$cotas],[new TLabel("1º Vencimento:", '#FF0000', '14px', null, '100%'),$dtvencimento]);
        $row1->layout = [' col-sm-7',' col-sm-2',' col-sm-3'];

        $row2 = $this->form->addFields([$bcontainer_643b11be2b6b2]);
        ...
        $row3 = $bcontainer_643b14df075e9->addFields([$instrucao]);
        $row3->layout = [' col-sm-12'];

        $row4 = $this->form->addFields([$bcontainer_643b14df075e9]);
        ...
        $row5 = $this->detailFormFaturadetalheFkIdfatura->addFields([new TLabel("Item:", '#ff0000', '14px', null, '100%'),$faturadetalhe_fk_idfatura_idfaturadetalheitem,$button__faturadetalhe_fk_idfatura],[new TLabel("Quantidade:", '#FF0000', '14px', null, '100%'),$faturadetalhe_fk_idfatura_qtde,$faturadetalhe_fk_idfatura_idfaturadetalhe],[new TLabel("Valor R$:", '#FF0000', '14px', null, '100%'),$faturadetalhe_fk_idfatura_valor]);
        $row5->layout = [' col-sm-6','col-sm-3',' col-sm-3'];

        $row6 = $this->detailFormFaturadetalheFkIdfatura->addFields([new TLabel("Lançamento (Conta Analítica):", null, '14px', null),$faturadetalhe_fk_idfatura_idpconta],[new TLabel("Ação:", null, '14px', null, '100%'),$button_adicionar_item_a_fatura_faturadetalhe_fk_idfatura]);
        $row6->layout = [' col-sm-9','col-sm-3'];

        $row7 = $this->detailFormFaturadetalheFkIdfatura->addFields([new THidden('faturadetalhe_fk_idfatura__row__id')]);
        ...
        $column_faturadetalhe_fk_idfatura_tipopagamento_transformed = new TDataGridColumn('tipopagamento', "Tipo", 'center');
        ...
        $column_calculated_2 = new TDataGridColumn('=( {valor} * {qtde} )', "Total", 'right');
        ...
        $column_calculated_2->enableTotal('sum', 'Total a Pagar R$', 2, ',', '.');

        $action_onEditDetailFaturadetalhe = new TDataGridAction(array('FaturaContaPagLoteForm', 'onEditDetailFaturadetalhe'));
        ...
        $action_onDeleteDetailFaturadetalhe = new TDataGridAction(array('FaturaContaPagLoteForm', 'onDeleteDetailFaturadetalhe'));
        ...
        $this->faturadetalhe_fk_idfatura_list->addColumn($column_faturadetalhe_fk_idfatura_tipopagamento_transformed);
        ...
        $this->faturadetalhe_fk_idfatura_list->addColumn($column_calculated_2);
        ...
        $column_faturadetalhe_fk_idfatura_tipopagamento_transformed->setTransformer(function($value, $object, $row, $cell = null, $last_row = null)
        {
            //code here
        ...
        });        $row8 = $bcontainer_643b1563075ef->addFields([$this->detailFormFaturadetalheFkIdfatura]);
        $row8->layout = [' col-sm-12'];

        $row9 = $this->form->addFields([$bcontainer_643b1563075ef]);
        ...
        $btn_onshow = $this->form->addAction("Cancelar / Voltar", new TAction(['FaturaList', 'onShow']), 'fas:times #F44336');
        ...
        if(empty($param['target_container']))
        {
            $container->add(TBreadCrumb::create(["Financeiro","Conta a Pagar - Lançamento"]));
        }
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            $vencimento = TDate::date2us($param['dtvencimento']);

            for ($i = 1; $i <= $param['cotas']; $i++)
            {
            ...
            $object->dtvencimento = $vencimento;
            $vencimento = Uteis::nextMonth($vencimento);
