...
try{
  ...
  if( $situacaoImv!="" && ( $situacaoImv=="1,5" || $situacaoImv=="3,5" || $situacaoImv=="1" || $situacaoImv=="3" || $situacaoImv=="5" ) ){
  ...
  if(trim($buscaHero)== ""){
    ...
    $filtroHero = " AND
                   (upper(translate(imovel.imovelfull.uf,'ŠŽšžŸÁÇÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÜÏÖÑÝåáçéíóúàèìòùâêîôûãõëüïöñýÿ','SZszYACEIOUAEIOUAEIOUAOEUIONYaaceiouaeiouaeiouaoeuionyy')) like '%$buscaHeroMaiuscula%' OR
                    ...
                    upper(translate(imovel.imovelfull.imoveldestino,'ŠŽšžŸÁÇÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÜÏÖÑÝåáçéíóúàèìòùâêîôûãõëüïöñýÿ','SZszYACEIOUAEIOUAEIOUAOEUIONYaaceiouaeiouaeiouaoeuionyy')) like '%$buscaHeroMaiuscula%' OR
                    upper(translate(imovel.imovelfull.imoveltipo,'ŠŽšžŸÁÇÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÜÏÖÑÝåáçéíóúàèìòùâêîôûãõëüïöñýÿ','SZszYACEIOUAEIOUAEIOUAOEUIONYaaceiouaeiouaeiouaoeuionyy')) like '%$buscaHeroMaiuscula%' OR
                    upper(translate(imovel.imovelfull.imovelmaterial,'ŠŽšžŸÁÇÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÜÏÖÑÝåáçéíóúàèìòùâêîôûãõëüïöñýÿ','SZszYACEIOUAEIOUAEIOUAOEUIONYaaceiouaeiouaeiouaoeuionyy')) like '%$buscaHeroMaiuscula%' OR
                    upper(translate(imovel.imovelfull.caracteristicas,'ŠŽšžŸÁÇÉÍÓÚÀÈÌÒÙÂÊÎÔÛÃÕËÜÏÖÑÝåáçéíóúàèìòùâêîôûãõëüïöñýÿ','SZszYACEIOUAEIOUAEIOUAOEUIONYaaceiouaeiouaeiouaoeuionyy')) like '%$buscaHeroMaiuscula%' OR
  ...
  $cidadeImv = !isset($_GET["cidadeImv"])?0:$_GET["cidadeImv"];
  if(trim($cidadeImv)== "" || trim($cidadeImv)== "0"){
    $filtroCidade = "";
    $cidadeImv = 0;
    $_GET["bairroImv"] = 0;
    $coordCentral = $oConfig->latilongi;
  }else{
    $filtroCidade = " AND imovel.imovelfull.idcidade = $cidadeImv ";
    $sSelect = $pdo_imobi->query("SELECT latilongi
                                  FROM public.cidade
                                  WHERE public.cidade.idcidade = $cidadeImv
                                 ");
    $oCidadeCoord = $sSelect->fetch(PDO::FETCH_OBJ);
    $coordCentral = $oCidadeCoord->latilongi;
  }
  ...
  //Grid com Imóveis Paginados em 9
  ...
  $oImovelGrid = $sSelect->fetchAll(PDO::FETCH_OBJ);
  //Grid com Imóveis Totais
  ...
  foreach($oImovelGrid as $itemObjeto){
  ...
  fwrite($fp, '];');
  fclose($fp);
  $coordCentral = explode("@", $coordCentral);
  ...
  $sTemplate->assign("IMOVELGRID",$oImovelGrid);
  ...
  $sTemplate->display('imovelmap.tpl');
