...
<div class="property-list-2 p-2 bg-white property-block border transation-this hover-shadow rounded">
    <div class="overflow-hidden position-relative transation thumbnail-img bg-secondary hover-img-zoom">
        {if $GRIDIMOVELSHOW->etiquetanome|trim != ""}
            <div class="cata position-absolute"><span class="sale text-white {$ETIQUETAMODELO[$GRIDIMOVELSHOW->etiquetamodelo]}">{$GRIDIMOVELSHOW->etiquetanome}</span></div>
        {/if}
            ...
            {if $tem_img == true}
                ...
                {foreach from=$IMOVEL_IMAGES_EXPLODE item="IMAGEM_ITEM"}
                    ...
                    {if (stripos($file_headers[0],"404 Not Found") > 0  || (stripos($file_headers[0], "302 Found") > 0 && stripos($file_headers[7],"404 Not Found") > 0))}
                        {$filename = $URLSYSTEM|cat:$FILENOIMAGE}
                    {/if}
                    <div class="item card-transition" style="min-height: 24rem; background-image: url('{$filename}');background-size: cover;background-repeat: no-repeat;background-position: center center;" ></div>
                    {$XX = $XX+1}
                {/foreach}
            {else}
                <div class="item card-transition" style="min-height: 24rem; background-image: url('{$filename}');background-size: cover;background-repeat: no-repeat;background-position: center center;" ></div>
    ...
    <div class="property_text p-3">
        ...
        <span role="button" onclick="js_ImovelView({$GRIDIMOVELSHOW->idimovel},'{$nomeSeo}')">
            <span class="{$exibeValorSecundario}" style="position:absolute;right:25px;color:#ABB2B9;border-radius:5px;border:1px solid #ABB2B9;padding: 0 10px 0 10px;text-align:center">{$valorSecundario}</span>
            <h5 class="listing-title"><a href="javascript:void(0)">{$GRIDIMOVELSHOW->imoveltipo}</a></h5>
            <span class="listing-location"><i style="color: var(--theme-primary-color);" class="fas fa-map-marker-alt me-2"></i>{$GRIDIMOVELSHOW->bairro|lower|ucwords} - {$GRIDIMOVELSHOW->cidade|lower|ucwords} ({$GRIDIMOVELSHOW->uf})</span>
            {if $GRIDIMOVELSHOW->exibelogradouro == true}
                <span class="listing-location"><i style="color: var(--theme-primary-color);" class="fa-solid fa-location-arrow me-2"></i>{$GRIDIMOVELSHOW->endereco}</span>        
            {/if}
            <br>
            ...
            <div class="agent">
                <ul class="d-flex justify-content-between">
                    ...
                    <li><span>Construção</span><div class="text-dark">{$GRIDIMOVELSHOW->imovelmaterial}</div></li>
                </ul>
            </div>
            <hr>
            <style>
