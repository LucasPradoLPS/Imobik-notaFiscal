...
<div class="col mb-3" id="mapItem{$GRIDIMOVELSHOW->idimovel}">
                ...
                {if $tem_img == true}
                    ...
                    {foreach from=$IMOVEL_IMAGES_EXPLODE item="IMAGEM_ITEM"}
                        ...
                        {if (stripos($file_headers[0],"404 Not Found") > 0  || (stripos($file_headers[0], "302 Found") > 0 && stripos($file_headers[7],"404 Not Found") > 0))}
                            {$filename = $URLSYSTEM|cat:$FILENOIMAGE}
                        {/if}
                        <div class="item card-transition" style="min-height: 20rem; background-image: url('{$filename}');background-size: cover;background-repeat: no-repeat;background-position: center center;" ></div>
                        {$XX = $XX+1}
                    {/foreach}
                {else}
                    <div class="item card-transition" style="min-height: 20rem; background-image: url('{$filename}');background-size: cover;background-repeat: no-repeat;background-position: center center;" ></div>
